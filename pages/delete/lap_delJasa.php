...
session_start();
	...
	if(isset($_POST["id"])){
        ...
        $q2    = mysqli_query($conn, "SELECT * FROM galeri_jasa WHERE nomor_id = '$id' AND program = '$kategori' ");
        $i = 1;
        $sql2 = $q2;
        ...
        while ($data2 = mysqli_fetch_array($sql2)) {
            ...
            unlink("../../assets/images/laporan/jasa/".$total_dokumentasi[$i]);
        }
        unlink("../../assets/doc/jasa/".$excel);

		mysqli_query($conn, "UPDATE `input_jasa` SET 
        `tgl_laporan`='',
        `pemakaian`='',
        `dana_terpakai`='',
        ...
        mysqli_query($conn, "DELETE FROM `galeri_jasa` WHERE nomor_id = '$id' AND program = '$kategori'");

        $qPengajuan   = mysqli_query($conn, "SELECT * FROM input_jasa WHERE status = 'OK' AND laporan = 'Belum Laporan' ");
        $qPengajuan2  = mysqli_query($conn, "SELECT * FROM input_jasa WHERE status = 'OK' AND laporan = 'Belum Laporan' ");
