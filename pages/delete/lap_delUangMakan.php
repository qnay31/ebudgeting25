...
session_start();
	...
	if(isset($_POST["id"])){
        ...
        $q2    = mysqli_query($conn, "SELECT * FROM galeri_uang_makan WHERE nomor_id = '$id' AND program = '$kategori' ");
        $i = 1;
        $sql2 = $q2;
        ...
        while ($data2 = mysqli_fetch_array($sql2)) {
            ...
            unlink("../../assets/images/laporan/uang_makan/".$total_dokumentasi[$i]);
        }
        unlink("../../assets/doc/uang_makan/".$excel);

		mysqli_query($conn, "UPDATE `input_uang_makan` SET 
        `tgl_laporan`='',
        `pemakaian`='',
        `dana_terpakai`='',
        ...
        mysqli_query($conn, "DELETE FROM `galeri_uang_makan` WHERE nomor_id = '$id' AND program = '$kategori'");

        $qPengajuan   = mysqli_query($conn, "SELECT * FROM input_uang_makan WHERE status = 'OK' AND laporan = 'Belum Laporan' ");
        $qPengajuan2  = mysqli_query($conn, "SELECT * FROM input_uang_makan WHERE status = 'OK' AND laporan = 'Belum Laporan' ");
