...
$path   = '../../assets/images/laporan/logistik/'; // upload directory
$dir    = '../../assets/doc/logistik/';

$qExcel = mysqli_query($conn, "SELECT dokumen FROM input_logistik WHERE dokumen = '$nameExcel'");
...
if($_FILES['image'])
{
    foreach ($_FILES['image']['name'] as $id => $val) {
        ...
        $result = mysqli_query($conn, "INSERT INTO galeri_logistik (nomor_id, id_pengurus, program, nama, dokumentasi) VALUES (
            '$key', 
            '$id_pengurus', 
            '$pengajuan', 
            '{$dataLog[nama]}', 
...
$sql = "UPDATE input_logistik SET tgl_pemakaian = '$tglLaporan', deskripsi_pemakaian = '$pemakaian', dana_terpakai = '$terpakai', laporan = 'Menunggu Verifikasi', dokumen = '$nameExcel' WHERE id = '$key'";
...
$qPengajuan   = mysqli_query($conn, "SELECT * FROM input_logistik WHERE status = 'OK' AND laporan = 'Belum Laporan' ");
$qPengajuan2  = mysqli_query($conn, "SELECT * FROM input_logistik WHERE status = 'OK' AND laporan = 'Belum Laporan' ");
