...
session_start();
	...
	if(isset($_POST["id"])){
		...
		mysqli_query($conn, "UPDATE input_paudqu SET laporan = 'Terverifikasi' WHERE id = '$id' ");
        $i = 1;

        // Global Program
        $q  = mysqli_query($conn, "SELECT * FROM input_paudqu WHERE MONTH(tgl_pemakaian) = '$periode' AND laporan = 'Terverifikasi' ");
        $sql = $q;
        ...
        mysqli_query($conn, "UPDATE `data_paudqu` SET 
